...
level <int> ["Name"] { … }   # ONE STOREY of a multi-storey building — each level is its own complete drawing (page). A plan is either single-storey (no `level` block) or ALL levels: a drawable statement beside them is E_LEVEL_MIX. Settings (`paper`/`scale`/`grid`/`north`/`dims`/`title`/`axes`/`schedule`/`legend`), `component`/`import` and plan-global `let`/`set` stay OUTSIDE and apply to every level; one building = one sheet + one scale. Numbers are integers, unique (E_LEVEL_DUP), 0/negative legal (`level -1 "Basement"`), drawn ASCENDING — the lowest is page 1. Ids are unique WITHIN a level, so the same id on two levels is legal and means vertical identity (a stair). `arch compile` writes one file per level (`plan.L1.svg`, `plan.L2.svg`, …; `--level <n>` for just one) and `describe --json` reports `levels[]` (its top-level facts are the lowest storey). Plan-level block only
...
- **Settings / control:** `plan`, `component`, `let`, `theme`, `title`, `style`, `import`, `for`, `if`, `while`, `else`, `set`, `strip`, `level`, `axes`, `schedule`, `legend`
