 ...
 *
 * Written locally so the dataset layer adds no dependency. It produces standard
 * unified-diff text (`--- a`/`+++ b` headers, `@@ -l,c +l,c @@` hunks, ` `/`-`/`+`
 * lines) between two `.arch` sources — enough for a reader to see exactly what the
 * fix→repair trajectory rewrote. Deterministic and pure.
 */
