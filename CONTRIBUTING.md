...
### CJK font — `@chanmeng666/archlang-font-cjk` (npm, via the same workflow)

The Chinese/Japanese/Korean face the PDF and PNG backends use lives in `packages/font-cjk/` as a
**separately versioned** package. The core names it **exactly** in `optionalDependencies` (so
`npm i` / `npx` fetch it by default), and `release.yml` publishes it **before** the core. The
workflow fails before anything reaches npm if the pin and `packages/font-cjk/package.json`
disagree.

- **Its first version is published by hand, once.** npm offers the Trusted Publisher setting
  only on a package that already exists, and this repo never holds an npm token. So the owner
  runs, with 2FA: `npm publish -w packages/font-cjk --access public`, then configures
  npmjs.com → `@chanmeng666/archlang-font-cjk` → Settings → Trusted Publisher → GitHub Actions
  (`ChanMeng666/archlang`, `release.yml`). **No core release that pins it can ship before
  that.** Until then the font step fails with an auth error and stops the release before the
  core publish.
- **Regenerating the font** (rare: a new upstream Noto release or a coverage change):
  `pip install fonttools==4.63.0`, then `npm run gen:font-cjk`. The script downloads the pinned
  upstream TTC, verifies every input's SHA-256, subsets, renames and rewrites
  `ArchLangCJKSans-Regular.otf` plus its `.sha256`. It is deliberately **not** in `gen:all` /
  `check:drift` because it needs the network and Python. A changed font means a new package
  version **and** the same new exact version in the core's `optionalDependencies`.
