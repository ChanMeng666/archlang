...
| `test/docs-examples-figures.test.ts` | every headline figure on `docs-site/examples.md` re-derives from `describe()` on the plan that section's own `<ArchLive>` widget names. The page promises exactly this in its preamble and had drifted in ten places by 2026-09 — "twenty-six of the thirty-two catalogued furniture kinds" against a catalogue of 83, a two-bedroom flat two doors and two windows short. It parses only **bold** runs that START with a figure shape (`N rooms, N m²[, N doors][, N windows]`, or `N doors and N windows`), so a span like `**220.7 m² of ground**` is not a claim it reads and the false-positive rate is zero | fix the PROSE from the compiler's answer, never the plan to suit the prose. Scales, sheet sizes, footprints, zone subtotals and per-level figures are deliberately **not** gated — they sit in free-form prose where a number can be a counterfactual ("its box would claim 168") or a worked example ("halve that to 1:100"), and a gate that fires on prose is one that gets widened until it stops firing. Those stay a review item |
...
| `docs-site/e2e/page-chrome.spec.ts` — the nav band | on `/` **and** `/guide` at 768 / 800 / 900 / 959, `document.scrollWidth - clientWidth` is exactly 0; below 960 the hamburger is visible, the desktop row is not, and the drawer reaches all seven top-level destinations; at 960 the row is back with all seven. VitePress reveals the desktop nav at `min-width: 768px` and our eight-destination bar needs 960, so the site shipped 272px of sideways scroll at 768 on every route. Unlike the full-bleed-band gate below it this one does **not** skip under overlay scrollbars — the overhang is hundreds of pixels, not one scrollbar | the `@media (min-width: 768px) and (max-width: 959.98px)` block at the foot of `style.css` was lost or out-specified (each rule beats a VitePress `<style scoped>` (0,2,0) only through its `header.VPNav` prefix). **Known and separately tracked:** at >= 960 `.VPNav` is `position: fixed`, so the same too-wide row is CLIPPED instead of scrolling and `scrollWidth` reads 0 — the last nav item is cut off across [960, 1003] on `/` and [960, 1138] on a doc page. Do not widen this gate to cover it; see `docs/backlog.md` |
