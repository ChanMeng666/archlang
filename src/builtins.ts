...
function n(v: Value | undefined, onError: OnError, span?: Span): number {
  ...
  onError({ severity: "error", message: `Expected a number but got ${v ? typeName(v) : "nothing"}`, code: "E_TYPE", span });
  return 0;
}
...
function arity(name: string, args: Value[], want: number, onError: OnError, span?: Span): boolean {
  ...
  onError({ severity: "error", message: `"${name}" expects ${want} argument(s) but got ${args.length}`, code: "E_ARITY", span });
  return false;
}
...
const BUILTINS: ReadonlyMap<string, BuiltinFn> = new Map<string, BuiltinFn>([
  ["min", (a, e, s) => {
    if (a.length === 0) { e({ severity: "error", message: `"min" needs at least 1 argument`, code: "E_ARITY", span: s }); return num(0); }
    return num(Math.min(...a.map((v) => n(v, e, s))));
  }],
  ["max", (a, e, s) => {
    if (a.length === 0) { e({ severity: "error", message: `"max" needs at least 1 argument`, code: "E_ARITY", span: s }); return num(0); }
  ...
  ["sqrt", (a, e, s) => {
    ...
    const x = n(a[0], e, s);
    if (x < 0) { e({ severity: "error", message: `"sqrt" of a negative number`, code: "E_DOMAIN", span: s }); return num(0); }
  ...
  ["len", (a, e, s) => {
    ...
    e({ severity: "error", message: `"len" expects an array or string but got ${typeName(v)}`, code: "E_TYPE", span: s });
    return num(0);
  }],
