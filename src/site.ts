 ...
 * - **Without a host room** — either none was found, or the host is a POLYGON room, whose
 *   bounding box would answer for an edge the window is not on: the axis comes from the
 *   host wall segment's orientation (a horizontal segment → `N`/`S`, a vertical one →
 *   `E`/`W`), and the outward side is the half of the plan the window sits in relative to
 *   `planCenter` (above centre → `N`, left of centre → `W`). With no host segment either,
 *   the axis falls back to the window's dominant offset from `planCenter`.
 */
