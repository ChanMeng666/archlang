...
| Thing | Current | Where |
|-------|---------|-------|
| **Core package** | `@chanmeng666/archlang@1.27.0` (published, `latest`, with provenance — released tokenlessly via `.github/workflows/release.yml` OIDC trusted publishing) | npmjs.com/package/@chanmeng666/archlang |
...
| **Git** | `main`, tags `v1.0.0` → `v1.27.0` (latest; a `v*` tag push triggers the tokenless OIDC release workflow) | github.com/ChanMeng666/archlang |
...
**The prior release, v1.26.1 (2026-08-13)** — **"five shipped surfaces that no test ever executed"**, a
...
├─ examples/          27 plans, each the flagship of ONE thing (plus lib/ and the twelve committed
...
│                     two-bed · bungalow (the DOOR-KIND flagship) · courtyard-house (the CONCAVE
...
npm run gen:example-svgs   # re-render the twelve committed examples/*.svg the README embeds, from
