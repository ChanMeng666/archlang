...
**Latest release: v1.32.0 (2026-08-28)** — **"the furniture catalogue"**, a MINOR, **PREPARED
BUT NOT RELEASED**: the version bump, `CHANGELOG.md` entry, lockfile, MCP shim 0.2.12 and VS Code
0.21.0 are committed on `main`, but **no `v1.32.0` tag exists**, so npm is still 1.31.0, the MCP
registry still 0.2.11 and the gallery still 0.18.0. **No language change** — no new keyword, no new
...
inside the `.vsix`; the Marketplace upload is a human web step and has NOT been performed — 0.20.0
and 0.19.0 are un-uploaded too, so upload the HIGHEST).
...
0.20.0 was packaged for it and, like 0.19.0 and now 0.21.0, has never been uploaded. Every previous
...
upload is a human web step, and as of the v1.32.0 prep it still has NOT been performed for 0.20.0,
0.19.0 or 0.21.0 — upload the HIGHEST).
