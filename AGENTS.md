...
| Thing | Current | Where |
|-------|---------|-------|
| **Core package** | `@chanmeng666/archlang@1.30.0` — **PREPARED, NOT YET PUBLISHED.** The version bump, `CHANGELOG.md` entry and lockfile are committed on `main` (2026-08-28) but **no `v1.30.0` tag has been pushed**, so nothing has reached npm: the registry's `latest` is still **1.29.0** (probed 2026-08-28, `npm view @chanmeng666/archlang version` → `1.29.0`). Publishing happens when someone pushes the tag. Released tokenlessly via `.github/workflows/release.yml` OIDC trusted publishing, with provenance | npmjs.com/package/@chanmeng666/archlang |
...
| **Git** | `main`, tags `v1.0.0` → `v1.29.0`. **`v1.29.0` is still the newest tag that exists** — `main` has moved ahead of it with the v1.30.0 release commits, and **no `v1.30.0` tag has been created or pushed** (`git ls-remote --tags origin`). v1.29.0 points at `2b183ba` and its GitHub Release is dated 2026-08-26T11:51:00Z. A `v*` tag push triggers the tokenless OIDC release workflow | github.com/ChanMeng666/archlang |
...
├─ examples/          29 plans, each the flagship of ONE thing (plus lib/ and the nineteen committed
...
npm run gen:example-svgs   # re-render the nineteen committed examples/*.svg the README embeds, from
...
- **Don't edit `dist/` or generated files.** `dist/` is a build output. The generated artifacts —
  ...
  `schemas/*.schema.json`, and **the nineteen committed `examples/*.svg`** — each come from a single
