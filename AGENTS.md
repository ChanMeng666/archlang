...
| Thing | Current | Where |
|-------|---------|-------|
| **Core package** | `@chanmeng666/archlang@1.28.0` — **version bumped and committed, NOT yet published**: `latest` on the registry is still **1.27.0** until a `v1.28.0` tag push runs the release workflow (probed 2026-08-26, `npm view @chanmeng666/archlang version` → `1.27.0`). Released tokenlessly via `.github/workflows/release.yml` OIDC trusted publishing, with provenance | npmjs.com/package/@chanmeng666/archlang |
...
| **Git** | `main`, tags `v1.0.0` → `v1.27.0` (the newest tag that EXISTS; `v1.28.0` is prepared but **not yet tagged** — a `v*` tag push triggers the tokenless OIDC release workflow) | github.com/ChanMeng666/archlang |
...
NOT been performed when this line was written). **The prior release, v1.27.0 (2026-08-25)** —
...
│                     two-bed · bungalow (the DOOR-KIND flagship) · furnished-flat (the FURNITURE
│                     flagship: 26 of the 32 catalogued kinds across all five symbol domains, no
│                     `label` on any of them and no `size` on most) · courtyard-house (the CONCAVE
...
│                     placed four times) · two-storey · accessible (accTitle/accDescr).
