...
| **VS Code extension** | `ChanMeng.archlang@0.8.0` **live on the Marketplace** (2026-07-26; rebundles core 1.19.0 — `flush`, `dim faces`/`clear`, `W_FIXTURE_BACK_TO_ROOM`/`W_DIM_INSIDE`/`W_DIM_NO_WALL`/`E_FURN_FLUSH`, stable-ref `suggest`). **0.10.0 is built and packaged but NOT yet uploaded** — `editors/vscode/archlang-0.10.0.vsix` rebundles core 1.21.0 (`level`/`stair`/`elevator`/`escalator`, `E_LEVEL_MIX`/`E_LEVEL_DUP`/`E_LEVEL_NEST`/`E_VERT_SIZE`/`E_STAIR_WIDTH`/`W_STAIR_UNMATCHED`, per-storey diagnostics, area-scaled circulation) and **supersedes 0.9.0, which was packaged for core 1.20.0 and never uploaded** — so this one upload carries both rounds' surface. The extension bundles the core at build time, and `.vsix` files are gitignored, so the artifact is local to whoever ran `npm run package`. Upload stays a human web step at marketplace.visualstudio.com/manage/publishers/ChanMeng (no CI publish exists) | marketplace.visualstudio.com/items?itemName=ChanMeng.archlang |
...
| **Git** | `main`, tags `v1.0.0` → `v1.21.0` (latest; a `v*` tag push triggers the tokenless OIDC release workflow) | github.com/ChanMeng666/archlang |
...
**Latest release: v1.21.0 (2026-07-26)** — Batch-2 sub-release **"vertical"**: `level <n> ["Name"]
{ … }` blocks make a plan a **set of drawings** — one complete sheet per storey, written to
`<stem>.L<n>.<ext>` with `pages[]`/`outputs[]` in `--json` and a `--level <n>` read
(`E_LEVEL_MIX`/`E_LEVEL_DUP`/`E_LEVEL_NEST`, ids unique per storey, one building = one paper/scale);
`stair` / `elevator` / `escalator` draw the conventional symbols and carry the one cross-floor rule —
the same id on two storeys is one shaft, surfaced as `describe().vertical` + per-storey `verticals`,
feeding upper-storey reachability and `validate --graph` (`E_VERT_SIZE`/`E_STAIR_WIDTH`/
`W_STAIR_UNMATCHED`); and the circulation/occupancy grids now size their cell from the plan's **area**
under a cell budget instead of a fixed cell count, so a 100 m building's clear widths discriminate
(dwelling-scale plans stay byte-for-byte identical). New `compile().pages`, `describe().levels`,
`Diagnostic.level`, and `examples/two-storey.arch`. (v1.20.0, 2026-07-26, was the "sheet & datum"
sub-release — `paper`/`axes`/`schedule`/`legend` and the operative drawing scale.) The table above is what is live. Canonical release notes
