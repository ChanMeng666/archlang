...
**This file is the MAP and it stands alone** — every project fact lives here: shipped state, iron
laws, commands, architecture, gotchas, layout. `CLAUDE.md` beside it is the Claude Code **operating
brief** and asserts no fact of its own: a ranked checklist of the rules most often broken here, each
pointing back into this file, plus the procedure for proving a change is right. **A new project fact
goes HERE, once.** If the same rule ends up stated in both files, delete the `CLAUDE.md` copy and
leave a pointer — both are loaded into every Claude Code session, so a duplicated sentence is paid
for twice and drifts silently.
...
laws" just below — read *those*, not the archive, for what still binds you.** Older pre-launch docs
...
stated at the foot of this file and has now been violated, and repaired, twice.
...
- Ongoing release narrative goes in `CHANGELOG.md` only — do not re-grow per-release prose here. It
  has been re-grown and re-archived twice now; the archives live outside this repo, at
  `archcanvas-growth/archive/archlang/docs-archive/agents-status-history-2026-07.md` (v1.3.0 →
  v1.16.0) and `…/agents-status-history-2026-09.md` (v1.20.0 → v1.35.0). **This file is loaded into
  every session, so treat 150,000 characters as a hard budget** — measure it (`wc -c AGENTS.md`)
  whenever you add a paragraph to the status table, and archive rather than accumulate.
