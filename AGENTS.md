...
| **VS Code extension** | `ChanMeng.archlang@0.7.0` is the version **live on the Marketplace** (2026-07-12, bundles core 1.15.0). **0.8.0 is built and packaged but NOT yet uploaded** — `editors/vscode/archlang-0.8.0.vsix` rebundles core 1.19.0 (`flush`, `dim faces`/`clear`, `W_FIXTURE_BACK_TO_ROOM`/`W_DIM_INSIDE`/`W_DIM_NO_WALL`/`E_FURN_FLUSH`, stable-ref `suggest`); the upload is a human web step at marketplace.visualstudio.com/manage/publishers/ChanMeng (no CI publish exists) | marketplace.visualstudio.com/items?itemName=ChanMeng.archlang |
