...
| Thing | Current | Where |
|-------|---------|-------|
| **Core package** | `@chanmeng666/archlang@1.30.0` — **PUBLISHED and live** (2026-08-28; npm `latest` 1.30.0, carrying a SLSA provenance attestation). Tag `v1.30.0` points at `5298b99` and its GitHub Release is dated 2026-08-28T08:06:04Z. **The release workflow's FIRST run failed** — at `mcp-publisher publish`, because the registry's validation of `server.json` against the just-published npm package 404'd on a version that same job had published moments earlier; `gh run rerun --failed` succeeded with no change, which is the signature of a race and not of a bad manifest. That step now carries a bounded retry (6 attempts, 20 s apart) — see `docs/backlog.md` 4.7. Released tokenlessly via `.github/workflows/release.yml` OIDC trusted publishing, with provenance | npmjs.com/package/@chanmeng666/archlang |
...
| **Git** | `main`, tags `v1.0.0` → `v1.30.0` (the newest tag that exists and is pushed — `git ls-remote --tags origin`; it points at `5298b99`, and its GitHub Release is dated 2026-08-28T08:06:04Z). v1.29.0 points at `2b183ba`. A `v*` tag push triggers the tokenless OIDC release workflow | github.com/ChanMeng666/archlang |
...
— the registry step only after a re-run, having lost a race with npm's own visibility. **VS Code
0.19.0 remains PACKAGED and NOT uploaded**, because that step is a human one. **No language change** — no new keyword, no new `E_*`/`W_*` code, nothing removed from the
...
performed). **The prior release, v1.29.0 (2026-08-26)** — **"two drawing-only elements, four furniture families, and
...
own name de-anonymises it, so `check:drift` now gates 29 artifacts across 9 generators (22 at the time of the extraction; the README SVG list grew from 13 to 19 in the 2026-08-28 gallery refresh) and there is
