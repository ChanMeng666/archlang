...
| Thing | Current | Where |
|-------|---------|-------|
| **Core package** | `@chanmeng666/archlang@1.31.0` — **PREPARED, NOT YET PUBLISHED.** The version bump, `CHANGELOG.md` entry and lockfile are committed on `main` (2026-08-28) but **no `v1.31.0` tag has been pushed**, so nothing has reached npm: the registry's `latest` is still **1.30.0** (probed 2026-08-28, `npm view @chanmeng666/archlang version` → `1.30.0`). Publishing happens when someone pushes the tag. The PREVIOUS release, **1.30.0, is live** — tag `v1.30.0` at `5298b99`, GitHub Release 2026-08-28T08:06:04Z, npm `latest` 1.30.0 with a SLSA provenance attestation. Its workflow's FIRST run failed at `mcp-publisher publish` (the registry's validation of `server.json` 404'd on a version that same job had published moments earlier; `gh run rerun --failed` then succeeded with no change — the signature of a race, not a bad manifest), which is why that step now carries a bounded retry of 6 attempts 20 s apart (`docs/backlog.md` 4.7, shipping in v1.31.0). Released tokenlessly via `.github/workflows/release.yml` OIDC trusted publishing, with provenance | npmjs.com/package/@chanmeng666/archlang |
...
| **Git** | `main`, tags `v1.0.0` → `v1.30.0`. **`v1.30.0` is still the newest tag that exists** — `main` has moved ahead of it with the v1.31.0 release commits, and **no `v1.31.0` tag has been created or pushed** (`git ls-remote --tags origin`). v1.30.0 points at `5298b99` and its GitHub Release is dated 2026-08-28T08:06:04Z; v1.29.0 points at `2b183ba`. A `v*` tag push triggers the tokenless OIDC release workflow | github.com/ChanMeng666/archlang |
...
upload is a human web step and has NOT been performed — 0.19.0 is un-uploaded too, so upload the
HIGHEST).

**The prior release, v1.30.0 (2026-08-28)** — **"one boundary for a set of walls, and the optional
