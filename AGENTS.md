...
| **VS Code extension** | `ChanMeng.archlang@0.10.0` **live on the Marketplace** (uploaded 2026-07-26, verified via the gallery API — it is the only version the gallery lists; bundles core 1.21.0 — `level`/`stair`/`elevator`/`escalator`, `E_LEVEL_MIX`/`E_LEVEL_DUP`/`E_LEVEL_NEST`/`E_VERT_SIZE`/`E_STAIR_WIDTH`/`W_STAIR_UNMATCHED`, per-storey diagnostics, area-scaled circulation — and carries 0.9.0's core-1.20.0 sheet/datum surface, which was packaged but never uploaded). The extension bundles the core at build time, and `.vsix` files are gitignored, so the artifact is local to whoever ran `npm run package`. Upload stays a human web step at marketplace.visualstudio.com/manage/publishers/ChanMeng (no CI publish exists) | marketplace.visualstudio.com/items?itemName=ChanMeng.archlang |
