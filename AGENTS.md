...
| Thing | Current | Where |
|-------|---------|-------|
| **Core package** | `@chanmeng666/archlang@1.29.0` — **version bumped and committed on `feat/v129`, NOT yet published**: `latest` on the registry is **1.28.0** until a `v1.29.0` tag push runs the release workflow (probed 2026-08-26, `npm view @chanmeng666/archlang dist-tags --json` → `{"latest":"1.28.0"}`, and that publish carries a SLSA provenance attestation). Released tokenlessly via `.github/workflows/release.yml` OIDC trusted publishing, with provenance | npmjs.com/package/@chanmeng666/archlang |
...
| **MCP server** | `@chanmeng666/archlang-mcp@0.2.9` (prepared for the v1.29.0 tag — NOT yet on the registry; published `latest` is **0.2.8**, which the v1.28.0 tag did ship, and the registry entry `io.github.ChanMeng666/archlang-mcp` reads v0.2.8 until the next tag push runs the release workflow. Probed 2026-08-26: `npm view @chanmeng666/archlang-mcp versions --json` ends `…"0.2.7","0.2.8"`. **Probe the MCP registry with `?version=latest`** — the bare `GET /v0/servers?search=…` returns the FIRST-published entry, which for this server is `0.1.1` from 2026-07-10, carrying its long-dead `archlang-docs.vercel.app` website URL — reading that as "the registry never got the updates" is wrong; `…&version=latest` returns `0.2.8`, `updatedAt` 2026-08-26T09:17:06Z, the same minute as the GitHub Release; `packages/mcp/`; stdio shim over the library; tools compile/describe/lint/validate (incl. `intent`)/**score**/repair/fix/suggest/complete + spec/context/schema/**intent-schema**/grammar resources; SDK dep quarantined here, core stays zero-dep). **Its context resources are baked in at pack time and only a version bump ships fresh ones** — the staleness itself is no longer silent: CI's `builds` job runs `packages/mcp/scripts/check-dist-resources.mjs` (byte-compares every baked `dist/` resource against its repo source) and `packages/mcp/test/lockstep.test.ts` pins the core dep range as a **string** equal to `^` + the root version, so every core release turns this package RED on purpose until someone re-pins, rebuilds the resources and bumps the shim in both `package.json` and both of `server.json`'s version fields (don't relax it to a semver-satisfies check). History, one clause: 0.2.2 served the v1.19 spec *and a v1.19 GBNF grammar that could not decode* `paper`/`level`/`place`/`zone`/`polygon`/`arc` while its `^1.14.0` range resolved to a current core; 0.2.3 refreshes all five, pins the range to `^1.24.0`, returns every storey of a multi-storey `compile` in `pages[]` (+ a `level` selector) instead of the ground floor alone, and derives the handshake version from `package.json` (the old hardcoded `"0.2.0"` drift, now test-pinned); **0.2.4** re-pins to `^1.25.0` and rebakes all five resources for the `site` layer and the door kinds — the shipped sources did not change at all, and the bump exists purely to ship those refreshed resources, which is exactly the case the pack-time law describes; **0.2.5** is the same case again and the strongest instance of it yet — three of the five baked resources (`spec.llm.md`, `llms-full.txt` *and* `grammars/archlang.gbnf`) changed at once in v1.26.0, so published 0.2.4 was handing hosts the wrong furniture syntax **and** a grammar that lets a constrained decoder emit uncompilable output; **0.2.6** is the pack-time law a third consecutive time, and the cheapest possible instance of it — exactly ONE of the five baked resources moved in v1.26.1 (`llms-full.txt`, carrying `W_DIM_INSIDE`'s corrected fix prose), the shipped sources are unchanged again, and published 0.2.5 would have kept telling hosts that a dimension reading inside always has an automatic fix; re-pinned to `^1.26.1`; **0.2.7** is the pack-time law a FOURTH consecutive time and the largest instance since 0.2.3 — three of the five baked resources moved in v1.27.0 (`spec.llm.md`, `llms-full.txt` *and* `grammars/archlang.gbnf`, all carrying the `on <wall> at <pos>` expression form plus `E_PARSE`), the shipped `packages/mcp` sources are again unchanged (`git diff v1.26.1..main -- packages/mcp` is empty), and published 0.2.6 would keep handing hosts a GBNF that refuses the one form a `for`-generated run of openings needs; re-pinned to `^1.27.0`; **0.2.8** is the pack-time law a FIFTH consecutive time — two of the five baked resources moved in v1.28.0 (`spec.llm.md` and `llms-full.txt`, carrying the widened catalogued-footprint fixture list that `spec.llm.md` interpolates from `CANONICAL_FIXTURES`), while `grammars/archlang.gbnf` and both schemas are **byte-unchanged**, which is correct: the furniture vocabulary is a catalogue, not a grammar token, so a constrained decoder was never wrong here. The shipped `packages/mcp` sources are unchanged for the fifth release running, and published 0.2.7 would keep serving the pre-furniture footprint list — eight plumbing/kitchen fixtures and an ellipsis, where the form now names **eighteen** categories that may omit `size` under `against wall`; re-pinned to `^1.28.0`; **0.2.9** is the pack-time law a SIXTH consecutive time, and the first since 0.2.7 where the GRAMMAR moved — three of the five baked resources changed in v1.29.0 (`spec.llm.md`, `llms-full.txt` *and* `grammars/archlang.gbnf`), measured by unpacking the PUBLISHED 0.2.8 tarball and SHA-256'ing each baked file against the repo tree rather than by diffing the tag. `roof` and `void` are real tokens, so published 0.2.8's GBNF cannot DERIVE either statement: a constrained decoder pointed at it is not merely mis-taught, it is unable to emit the new syntax at all. Both schemas are byte-identical and correctly so — neither element enters Plan JSON. The shipped `packages/mcp` sources are unchanged for the sixth release running; re-pinned to `^1.29.0` | `packages/mcp/`, `server.json` |
| **VS Code extension** | **`0.18.0` — PACKAGED 2026-08-26 against core 1.29.0, NOT YET UPLOADED.** The `.vsix` exists at `editors/vscode/archlang-0.18.0.vsix` (463.2 KB, 12 files) and was verified from INSIDE the archive: `Identity Version="0.18.0" Publisher="ChanMeng"` in `extension.vsixmanifest`, manifest `0.18.0` with dep `^1.29.0`, and `ARCHLANG_CORE_VERSION = "1.29.0"` the only core version literal in `dist/server.js` (an unquoted `\b1\.\d+\.\d+\b` scan of the whole bundle returns exactly two matches, and reading their context shows the other is the unchanging `"@scope/name:1.0.0"` example inside the import-diagnostic message — scan unquoted and read the context, since a `"1\.\d+\.\d+"` regex misses that one entirely and flatters the result). The new surface is present in the bundled server, not merely in the repo: `"roof"` ×20, `"void"` ×14, `E_ROOF_AMBIGUOUS`/`E_ROOF_CURVED`/`E_ROOF_SELF_INTERSECT`/`E_VOID_SIZE` ×3–4 each, and the four new fixture families with their aliases (`rug`/`carpet`, `piano`/`grand_piano`, `sofa_l`, `sun_lounger`) plus `underlay`, so completion, hover and the fixture lint rules all know them. **The Marketplace upload is a human web step** at marketplace.visualstudio.com/manage/publishers/ChanMeng — nothing in this repo performs it, so until someone does, the gallery serves 0.17.0 and that is not a failure. History: **`0.17.0` — LIVE, uploaded 2026-08-26** against core 1.28.0 (the 51-category fixture vocabulary and its symbols). **Confirmed** via the gallery API (`extensionquery`, `filterType: 7` = `ChanMeng.archlang`): `0.17.0` is the only version returned, `lastUpdated` 2026-08-26T09:30:34Z — ~13 minutes after the v1.28.0 GitHub Release, and the query agreed on the first probe, so the growing-lag trend recorded below is not monotonic. **`0.16.0` was never uploaded and is now doubly superseded** — packaged for v1.27.0, skipped, and its `.vsix` may still be sitting beside the newer ones (all are gitignored). Always upload the HIGHEST version. History: **`0.15.1` — LIVE, uploaded 2026-08-13** against core 1.26.1. **Confirmed** via the gallery API (`extensionquery`, `filterType: 7` = `ChanMeng.archlang`): `0.15.1`, `lastUpdated` 2026-08-13T00:49:57Z; the `.vsix` was verified from INSIDE the archive first — manifest `0.15.1`, `1.26.1` its ONLY core version literal, and the release's fixes present (`dimReadsInside` x4, `doorKind` x23). The gallery lag was ~40 minutes this time (it was ~25 for 0.15.0 and ~10 for 0.14.0) — it is growing, so a disagreeing probe means "wait and re-probe", never "the upload failed". **Build and package in the PRIMARY checkout only** — a `.claude/worktrees/*` checkout has no `node_modules`, so esbuild resolves the core by walking UP to the shared repo and bundles THAT one, while `__CORE_VERSION__` still passes because both stamp the same version (`docs/backlog.md` item 3.14). No new syntax and no new code, but its diagnostics and quick fixes differ: nine core `src/` files changed and four are on the bundled server's path — `W_DIM_INSIDE` no longer offers the quick fix that walked the file in a 2-cycle, and `Format Document` no longer strips a door's kind and `slide`/`open` clauses. **Its dep range is now guarded** by `editors/vscode/test/lockstep.test.ts` (new in this release, mirroring the shim's: a **string** equality against `^` + the root version), which is what closed the hole below — the guard deliberately does NOT also require the extension's own version to bump, since its cadence is human Marketplace uploads, and coupling them would train people to bump a digit for green. **A build/package hazard that post-dates 0.15.0 — now GUARDED (v1.27.0):** never run `npm run package` or `vscode:build:only` from a `.claude/worktrees/*` checkout — a worktree has no `node_modules`, so esbuild walks UP and bundles the SHARED repo's core, and the `__CORE_VERSION__` stamp cannot catch it because both stamp the same version (`docs/backlog.md` item 3.14). The build now REFUSES instead of relying on the convention: `editors/vscode/resolve-core.mjs` compares the resolved core's real path against the repo root of the tree being built and throws naming both paths (`editors/vscode/test/wrong-core.test.ts`, which simulates the two-checkout walk-up on disk at the SAME version, so the case the stamp cannot see is the case the test asserts). It fires for a **junctioned** worktree too, and that is right, not over-strict: npm links a workspace package by ABSOLUTE path to the main tree's root, so junctioning `node_modules` moves the walk one step and still bundles the other checkout's core. History: **`0.15.0` — LIVE, uploaded 2026-08-12** against core 1.26.0 (`E_ROOM_ALIGN`/`E_ROOM_ALIGN_AXIS` with their quick fixes). **Confirmed** via the gallery API (`extensionquery`, `filterType: 7` = `ChanMeng.archlang`): `0.15.0`, `lastUpdated` 2026-08-12T11:55:36Z; the `.vsix` was verified from INSIDE the archive first — manifest `0.15.0`, `1.26.0` its ONLY core version literal, `E_ROOM_ALIGN` ×7 and `E_ROOM_ALIGN_AXIS` ×3 in `dist/server.js`. Two mechanics worth carrying: (1) **"rebundled" and "packaged" are different steps** — `npm run build` produces `dist/server.js`, only `npm run package` produces a `.vsix`, and v1.26.0's prep did the first and not the second, so no artifact existed until someone checked; (2) `npm run package` esbuilds against the CORE's `dist/`, so **`npm run build` at the repo root must run first** or it silently packages a stale language (reproduced live during this release: a bundle built before the version bump stamped `1.25.0` and the freshness test caught it). Its `@chanmeng666/archlang` range was **`^1.24.0` — two releases stale** — when v1.26.0 was prepared, because unlike the shim's that range had no lockstep test; that guard has since LANDED (v1.26.1) and the range is re-pinned each release. **The upload lag is real and grew:** the gallery still returned `0.14.0` ~25 minutes after this upload before flipping (it was ~10 minutes for 0.14.0), so a disagreeing probe means "wait and re-probe", never "the upload failed". History: `0.14.0` — built and uploaded 2026-08-11 against core 1.25.0 (`site`/`street`/`hemisphere`, the five door kinds with `slide`/`open`, the nine new codes, and the bbox-derived-position fixes; the `.vsix` was verified from INSIDE the archive — manifest `0.14.0`, `1.25.0` its only core version literal, and `equator_side`/`W_POCKET_RUN`/`E_SITE_DUP` present in `dist/server.js`), **confirmed live** via the gallery API (`extensionquery`, `filterType: 7` = `ChanMeng.archlang`): `0.14.0`, `lastUpdated` 2026-08-11T03:47:51Z. Worth knowing for next time: the gallery still returned `0.13.0` on five probes across the ~10 minutes AFTER the upload, then flipped — Marketplace validation lags, so a probe that disagrees with a just-performed upload means "wait and re-probe", not "the upload failed" (the same lag is recorded below: a 2026-08-01 probe saw 0.10.0 when 0.13.0 was already up). History: `ChanMeng.archlang@0.13.0` **live** — verified 2026-08-01 via the gallery API (`extensionquery`, `filterType: 7` = `ChanMeng.archlang`): **0.13.0 is the only version the gallery returns**, even with `IncludeAllVersions`; its `lastUpdated` is 2026-07-26T11:51:11Z, so the upload landed that day, shortly after the query that still saw 0.10.0. It bundles core 1.24.0 (`arc`/`radius`/`circle`/`cw`/`ccw`/`major`, `E_ARC_RADIUS`/`E_ROOM_RADIUS`/`E_DIM_CURVE_REF`, exact πR² areas and arc-length opening attribution in the bundled analysis) and carried in one upload the three tiers that were packaged but never uploaded: 0.12.0 (core 1.23.0 — `polygon`), 0.11.0 (core 1.22.0 — `zone`/`place`/`mirror`) and 0.9.0 (core 1.20.0 — sheet/datum). Durable mechanics: the extension **bundles the core at build time** (a stale bundle ships a stale language — `editors/vscode/test/` pins bundle freshness), `.vsix` files are gitignored so the artifact is local to whoever ran `npm run package`, and **upload stays a human web step** at marketplace.visualstudio.com/manage/publishers/ChanMeng — there is no CI publish | marketplace.visualstudio.com/items?itemName=ChanMeng.archlang |
...
| **Git** | `main`, tags `v1.0.0` → `v1.28.0` (the newest tag that EXISTS and is pushed — `git ls-remote --tags origin`, with a GitHub Release dated 2026-08-26T09:17:07Z; `v1.29.0` is prepared on `feat/v129` but **not yet tagged** — a `v*` tag push triggers the tokenless OIDC release workflow) | github.com/ChanMeng666/archlang |
...
resources moved, the GBNF among them) and **VS Code 0.18.0** (packaged and verified from inside the
`.vsix`; the Marketplace upload is a human web step and had NOT been performed when this line was
written). **The prior release, v1.28.0 (2026-08-26)** — **"the furniture vocabulary the examples were
...
│                     flagship: 26 of the 32 catalogued kinds across all five symbol domains, no
│                     `label` on any of them and no `size` on most, plus the four v1.29 families
│                     and the `underlay` rug) · courtyard-house (the CONCAVE
