...
const ELEMENT_GRAMMAR: Record<string, string> = {
  ...
  dim: 'dim [faces|clear] (x,y)->(x,y) offset <mm> [text "…"]   # a dimension line. Endpoint ORDER + the offset sign choose which side it lands on (the offset runs along the LEFT normal of from→to), so a reversed pair draws it INSIDE the building — `W_DIM_INSIDE`. `faces` pushes each endpoint out onto the wall it runs into (a true outside-to-outside span, no half-thickness arithmetic); `clear` pulls both in to the inner faces (a clear width). Or skip hand dims entirely: `dims auto all` draws the GB/T openings + axis + overall chains outside every dimensioned facade',
  column: "column [id=<name>] at (x,y) size <W>x<H>",
  stair:
    "stair [id=<name>] at (x,y) size <W>x<H> dir up|down [width <mm>]   # a flight: treads, a mid-flight break line, an UP/DN arrow. `at` = footprint TOP-LEFT; the flight runs along the LONG axis; `dir up` is entered at that axis's larger-coordinate end (arrow points N/W), `dir down` at the opposite end (arrow reversed) — so one shaft reads correctly on both floors. `dir` is declared per storey. MULTI-STOREY: the SAME id on two `level` blocks is ONE SHAFT — it becomes a `describe().vertical` connection and makes the upper storey reachable with no front door of its own (an id on one storey only = `W_STAIR_UNMATCHED`)",
  elevator:
...
const STATEMENT_GRAMMAR: Record<string, string> = {
  ...
  level:
    'level <int> ["Name"] { … }   # ONE STOREY = one whole drawing. A plan is single-storey or ALL levels (a drawable statement beside them = E_LEVEL_MIX); settings/`component`/`import`/plan-global `let`/`set` stay OUTSIDE, applying to every level (one building = one sheet + scale). Integers, unique, 0/negative legal, ASCENDING — lowest = page 1. Ids unique WITHIN a level, so one id on two floors is legal (a stair). `arch compile` writes plan.L1.svg, plan.L2.svg … (`--level <n>` = one); `describe --json` adds `levels[]`. Plan-level only',
  strip:
